...
(function () {
  ...
  function validate(name, content) {
    ...
    try {
      ...
      // hazard: only a real CLOSING script tag or an HTML comment-open marker breaks
      // single-file inlining (the same two tokens build.sh greps for). A bare OPENING
      // script tag is NOT a hazard -- script content is raw-text and only ends at a
      // closing tag -- and it legitimately appears in comments across modules, so
      // flagging it would block almost every edit. (The obscure script-data double-
      // escape needs a preceding comment-open marker, which we block below, so a lone
      // opening tag is safe.)
