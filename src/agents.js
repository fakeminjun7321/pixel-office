...
(function () {
  ...
  Agents.chat = function (agent, userText, opts) {
    ...
    // Serialize per-agent chat: abort any in-flight chat stream first, so a rapid
    // second message can't interleave two responses into the same bubble/transcript.
    if (agent._chatStream && typeof agent._chatStream.abort === 'function') {
      try { agent._chatStream.abort(); } catch (e) {}
    }
    agent._chatStream = null;
    ...
    var handle = App.API.stream({
      ...
      system: (agent.systemPrompt || '') +
        '\n\n[CHAT MODE] You are chatting 1:1 with a teammate. Reply IN CHARACTER and KEEP IT SHORT -- ' +
        '2-5 sentences or a brief bullet list; do not over-explain or dump a long guide. Understand ' +
        'everyday/social messages too (relationships, mood, small talk, breaks, getting along with the ' +
        "boss), not only work tasks. Reply in the user's language (Korean or English).",
      ...
      onDone: function (res) {
        ...
        if (agent._chatStream === handle) agent._chatStream = null;
      },
      onError: function (err) {
        // Superseded by a newer chat message (aborted) -> stay silent so the two
        // responses don't interleave; the newer stream keeps running.
        if (err && err.type === 'abort') { if (agent._chatStream === handle) agent._chatStream = null; return; }
        if (agent._chatStream === handle) agent._chatStream = null;
        agent.busy = false;
        Agents.setState(agent, 'idle');
